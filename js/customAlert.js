...
// simple comment
const mainHtml = `<div id='customAlert' ></div>`;
const mainStyle = `
<style>
#customAlert {
  z-index: 99999;
...
#customAlert.mini-customAlertObj {
  height: 150px;
  top: 0;
  left: 0;
  margin: 0;
...
#customAlert.mini-customAlertObj .sgr-mini {
...
#customAlert.mini-customAlertObj .sgr-mini .sgr-head {
  width: 100%;
  height: 50%;
  margin-bottom: 0.3em !important;
}

#customAlert.mini-customAlertObj .sgr-mini .sgr-head .sgr-mini-heading {
  font-size: 20px;
}

#customAlert.mini-customAlertObj .sgr-mini .sgr-head svg {
  height: 100%;
  color: gray;
  fill: gray;
  stroke: gray;
...
#customAlert.mini-customAlertObj .sgr-mini.sgr-error {
  background: #f6d7da;
...
#customAlert.mini-customAlertObj .sgr-mini.sgr-success {
...
#customAlert.mini-customAlertObj .sgr-mini.sgr-warning {
...
#customAlert.mini-customAlertObj .sgr-mini.sgr-info {
...
#customAlert.mega-customAlertObj {
  width: 100vw;
  height: 100vh;
...
#customAlert.mega-customAlertObj.sgr-fade-out {
...
#customAlert.mega-customAlertObj .sgr-mega {
...
#customAlert.mega-customAlertObj .sgr-mega header {
  border-bottom: 1px solid gray;
  padding: 1em;
}

#customAlert.mega-customAlertObj .sgr-mega header .sgr-heading {
  font-size: 20px;
  color: #8868df;
}

#customAlert.mega-customAlertObj .sgr-mega .sgr-body {
  padding: 1em;
}

#customAlert.mega-customAlertObj .sgr-mega .sgr-body .sgr-details {
  font-size: 30px;
  font-weight: 300;
  color: #8868df;
}

#customAlert.mega-customAlertObj .sgr-mega .sgr-body .sgr-more-details {
  font-weight: bold;
  font-size: 13px;
}

#customAlert.mega-customAlertObj .sgr-mega footer {
...
#customAlert.mega-customAlertObj .sgr-mega footer button {
  padding: 0 1em;
  outline: none;
  border: none;
  border-radius: 0.3em;
...
#customAlert.mega-customAlertObj .sgr-mega footer button:hover {
...
class Mini {
  ...
  customAlertObj = document.getElementById("customAlert");
  template = `
  ...
  clearAllClass() {
    this.customAlertObj.removeAttribute("class");
  }
  changecustomAlertObj() {
    if (!this.customAlertObj.classList.contains("mini-customAlertObj")) {
      this.customAlertObj.classList.add("mini-customAlertObj");
    }
  }
  remove() {
    const self = this;
  ...
  render() {
    ...
    this.customAlertObj.innerHTML = template;
  }
}
...
function mega({ title = "", message = "", details = "", left, right }) {
 ...
 `;
  ...
  if (!title) throw new Error("customAlertObj Alert:title is required");
  if (!right) throw new Error("customAlertObj Alert:right is required");
  const customAlertObj = document.getElementById("customAlert");
  let state = ``;
  function changecustomAlertObj() {
    if (customAlertObj.classList.contains("mini-customAlertObj")) {
      customAlertObj.classList.remove("mini-customAlertObj");
    }
    if (!customAlertObj.classList.contains("mega-customAlertObj")) {
      customAlertObj.classList.add("mega-customAlertObj");
    }
  }
  const template = `
  ...
  `;
  function clearcustomAlertObj() {
    customAlertObj.removeAttribute("class");
  }
  function removeMega() {
    state = ``;
  }
  ...
  function render() {
    customAlertObj.innerHTML = state;
  }
  function fadeOut() {
    if (!customAlertObj.classList.contains("sgr-fade-out")) {
      customAlertObj.classList.add("sgr-fade-out");
    }
  }
  function finish() {
    fadeOut();
...
main();
window.__proto__.customAlertObj = function (data, details, type, duration) {
  const customAlertObjType = typeof data;
  if (customAlertObjType === "string") new Mini(data, details, type, duration);
  else if (customAlertObjType === "object") return mega(data);
};
